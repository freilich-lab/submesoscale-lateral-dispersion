...
from utils import calculate_acceleration, convert_velocity


# Load datasets
velocity_ds = xr.open_dataset('aviso_data.nc') # This is the dataset that contains the coarse resolution velocity data
calculated_model_params = pd.read_csv('calculated_model_params_2023.csv') # Calculated parameters for the model data
...
def solve_ode_sys(space_init, t_grid, eta_variance_u, eta_variance_v, params_for_ivp, num_trajectories=5):
    ...
    for _ in range(num_trajectories):
        ...
        sol_xc = solve_ivp(ode_system, [t_grid[0], t_grid[-1]], y_init, t_eval=t_grid, args=(params_for_ivp,), method="BDF")
        traj_x_vals.append(sol_xc.y[0])
        traj_y_vals.append(sol_xc.y[1])
    return np.mean(traj_x_vals, axis=0), np.mean(traj_y_vals, axis=0)
...
def calc_adv_traj(adv_params, space_init):
    try:
        ...
        sol_adv = solve_ivp(adv_sys, [t_grid[0], t_grid[-1]], space_init, t_eval=t_grid, args=(u_adv_interp, v_adv_interp), method="BDF")
        adv_x = sol_adv.y[0]
        adv_y = sol_adv.y[1]
