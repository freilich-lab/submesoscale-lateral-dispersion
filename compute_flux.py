...
start = 0.005  # Start of the range in day^{-1} 
end = 2  # End of the range in day^{-1}
num_values = 20 # How many different lambda values
HOURS_PER_DAY = 24
SECONDS_PER_HOUR = 3600

# Time range 
# Corresponding to the overall year of interest (for purposes of calculating turbulent velocity)
...
# Comment ouut the above and uncomment out the below function in order to have the reversed nutrient gradient instead 
...
def reaction_term(t, P, lambda_param, x_t, y_t, coastline):
    # Extract the slope (m) and intercept (b) from the coastline dictionary
    m = coastline['m']
    b = coastline['b']
    # Evaluate the current position using the interpolated functions
    x_val = x_t(t)
    y_val = y_t(t)
    ...
    try:
        ...
        for lamda in lamdas:
            ...
            sol = solve_ivp(reaction_term, [0, delta_t * len(lon)], [Nzero_start],
                            args=(lamda_s, x_t, y_t, coastline), method='Radau', t_eval=times) #Radau works slightly better than all the other built in methods tried
            N_t = sol.y[0]
...
def main(file_ext):
    ...
    with open(lsgs_data_path + file_ext + '-u-vel.pkl', 'rb') as f: # Load the velocity data, separately calculated
