...
UNIT_CONVERSION = 1000  # Conversion factor for fluxes from mu mol/kg to mu mol/m^3 in case the original data was in the former units
...
def set_paths(mode, figure, varying):
    if mode == 'original':
        ...
        drifter_filename = '0.171-0.165-0.702-0.691-flux.pkl'
        adv_filename = '0.066-0.083-1.321-1.948-flux.pkl' # based on parameters calculated from generating trajectories with un-modified AVISO data
        ...
        drifter_filename = '0.171-0.165-0.702-0.691-flux.pkl'
        adv_filename = '0.066-0.083-1.321-1.948-flux.pkl'
        dist_path_non_params = SAVE_PATH_DISTRIBUTIONS
        if figure == 'figure2':
...
def compute_averages(flux_dict):
    """Compute average and maximizing lambda values, returned as dictionaries."""
    average_flux = {k: {lamda: np.nanmean(v) for lamda, v in flux.items()} for k, flux in flux_dict.items()}
    max_lambda = {k: max(v, key=v.get) for k, v in average_flux.items()}
    max_average_flux = {k: v[max_lambda[k]] for k, v in average_flux.items()}
    return max_average_flux, max_lambda
...
def plot_lambda_flux_dist(lambda_flux_dict, param_title, dist_path_non_params):
    ...
    lambda_values = np.array(list(lambda_flux_dict.keys())) *24
    ...
    for lambda_value, flux_averages in lambda_flux_dict.items():
        color = colormap(norm(lambda_value*24))
        sns.kdeplot(flux_averages*1000, color=color, bw_adjust=0.5)
...
def plot_best_fit(lambda_values, agg_data, ratio, xlabel, title, dist_path, dist_param, mode):
    ...
    if mode=='original':
        if dist_param == 'tau':
            plt.ylim([0, 15]) # based on the actual data, to get ideal visualization (consistent scale but not too zoomed out or in)
        else:
            plt.ylim([-10, 50])
    else:
        if dist_param == 'tau':
...
def plot_flux_vs_lambda(aggregated_data, ratio_u, ratio_v, dist_path):
    ...
    norm = plt.Normalize(0.11, 0.18) #based on the range of parameters simulated
    cmap = plt.cm.PuBu
...
def main():
    ...
    for lambda_val, fluxes in drifter_flux_dict.items():
        ...
        drifter_data[lamda_adj].append(np.mean(fluxes)*UNIT_CONVERSION) 
    ...
    for lambda_val, fluxes in adv_flux_dict.items():
        lamda_adj = np.round(lambda_val*TIME_CONVERSION, 4)
        adv_data[lamda_adj].append(np.mean(fluxes)*UNIT_CONVERSION)
    ...
    if args.varying == 'sig':
        filename_suffix = '0.702-0.691-flux.pkl'     
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith(filename_suffix) and filename.startswith(filename_prefix):
                ...
                for lambda_val, fluxes in lambda_flux_dict.items():
                    lamda_adj = np.round(lambda_val*UNIT_CONVERSION, 4) 
                    aggregated_data[lamda_adj].append(np.mean(fluxes)*UNIT_CONVERSION) 
