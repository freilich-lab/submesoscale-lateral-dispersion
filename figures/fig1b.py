...
def calculate_distance_travelled(longitudes, latitudes):
    ...
    displacements = np.vstack((displacement_ew, displacement_ns)).T     # Combine displacements into a single array of vectors

    normal_vector = np.array([m, -1]) / np.sqrt(m**2 + 1)  # Calculate the unit normal vector to coast
    parallel_vector = np.array([1, m]) / np.sqrt(m**2 + 1) # Unit vector parallel to the coast
    ...
    if args.plot == 'distance':
        ...
        for filename in os.listdir(traj_path):
            if filename.endswith(filename_suffix_u) and filename.startswith(filename_prefix):
                ...
                file_path_v = file_path_u.replace('-u-vel.pkl', '-v-vel.pkl')  # assuming both u and v have similar prefixes
                try:
                    ...
                    print(f"Error: {e}. Could not find corresponding velocity file for: {filename}")
